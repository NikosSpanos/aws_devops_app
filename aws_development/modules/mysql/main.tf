...
resource "aws_db_parameter_group" "db_param_group_dev" {
  name   = "rds-pg-dev"
  family = "mysql5.7"
...
resource "aws_db_subnet_group" "db_subnet_dev" {
  name       = "db-ec2-subnet-group-development-environment"
  subnet_ids = [var.ec2_instance_subnet, var.ec2_instance_subnet_id2]

  tags = {
    Name = "DB subnet group for development environment"
  }
}

#MySQL Server
resource "aws_db_instance" "mysql_server_dev" {
  allocated_storage      = 5
  engine                 = "mysql"
  engine_version         = "5.7"
  ...
  parameter_group_name   = aws_db_parameter_group.db_param_group_dev.id
  ...
  db_subnet_group_name   = aws_db_subnet_group.db_subnet_dev.name
  storage_type           = "standard"

  tags = {
    Name = "development-mysql-server"
  }
}